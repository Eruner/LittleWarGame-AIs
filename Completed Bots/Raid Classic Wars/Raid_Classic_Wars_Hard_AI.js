	...
	if(!scope['BOT-' + me]){
		scope['BOT-' + me] = (function(){
			...
			function loadTechnologies(){
				DATA.TECH = {/* Tech Name, Current Cost, Cost increment */
			...
			/********** ADVANCE **********/
			function researchWolfs() {
				try{
					if(DATA.GOLD < DATA.TECH["Increase Wolf Rate"][0]){
						return;
					}
					scope.order("Increase Wolf Rate" ,[DATA.FORTRESS]);
					DATA.GOLD -= DATA.TECH["Increase Wolf Rate"][0];
					DATA.TECH["Increase Wolf Rate"][0] += DATA.TECH["Increase Wolf Rate"][1];
				}catch(Pokemon){
					console.log('Error during researchWolfs:\n'+Pokemon);
				}
			}
			function researchSnakes() {
				try{
					if(DATA.GOLD < DATA.TECH["Increase Snake Rate"][0]){
						return;
					}
					DATA.GOLD -= DATA.TECH["Increase Snake Rate"][0];
					scope.order("Increase Snake Rate" ,[DATA.FORTRESS]);
					DATA.TECH["Increase Snake Rate"][0] += DATA.TECH["Increase Snake Rate"][1];
				}catch(Pokemon){
					console.log('Error during researchSnakes:\n'+Pokemon);
				}
			}
			/********** MATH MAGIC **********/
