...
try{
	...
	if(!scope['BOT-' + me]){
		scope['BOT-' + me] = (function(){
			...
			/********** ADVANCE **********/
			function researchKnights() {
				if(DATA.GOLD >= DATA.TECH["Increase Knight Rate"][0]){
					scope.order("Increase Knight Rate" ,[DATA.FORTRESS]);
					DATA.TECH["Increase Knight Rate"][0] += DATA.TECH["Increase Knight Rate"][1];
					console.log('Increase Knight Rate');
				}
			}
			/********** MATH MAGIC **********/
