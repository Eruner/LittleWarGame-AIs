...
try{
	...
	if(!scope['BOT-' + me]){
		scope['BOT-' + me] = (function(){
			...
			/********** ADVANCE **********/
			function researchSoldiers() {
				if(DATA.GOLD >= DATA.TECH["Increase Soldier Rate"][0]){
					scope.order("Increase Soldier Rate" ,[DATA.FORTRESS]);
					DATA.TECH["Increase Soldier Rate"][0] += DATA.TECH["Increase Soldier Rate"][1];
					console.log('Increase Soldier Rate');
				}
			}
			/********** MATH MAGIC **********/
